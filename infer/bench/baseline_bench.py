...
NARRATOR_SYSTEM = (
    "你是一个黑暗文学风格的实时剧情叙事引擎，以第二人称「你」向玩家叙述。"
    "只做战术暗示，不直述裁定结果，不使用 markdown。"
)

ARBITER_SYSTEM = (
    "你是实时剧情生成游戏的裁定引擎。根据规则与双方面板裁定行动结果，"
    "严格输出 JSON：{\"result\": str, \"damage\": int, \"state_changes\": [...], \"narration_hints\": [...]}。"
)

# 游戏味填充段（约 120 字/段），按目标字数重复拼接模拟历史上下文
_FILLER = (
    "第{i}回合：夜幕下的城区雾气弥漫，敌对势力的身影在废墟间闪现。你的角色灵体化后撤，"
    "魔力回路的灼痛提醒你今夜已消耗过半。远处传来钟声，监督者对昨夜仓库区的爆炸保持沉默。"
    "隐蔽状态仍未解除，岸边残留的能量痕迹指向山腰的寺庙。"
)

GM_REPORT = (
    "【GM报告】回合17夜间：玩家(以太操控者)与剑士于跨江大桥遭遇狂战士。"
    "剑士 HP 62/100，魔力储备40%；狂战士狂暴等级 B，其操控者藏于300m外废楼。"
    "玩家剩余特殊指令2次。上回合玩家进行了身份揭示，已被敌方阵营观测。"
